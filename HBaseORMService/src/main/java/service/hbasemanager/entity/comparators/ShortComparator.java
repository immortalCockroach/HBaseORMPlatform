...
public class ShortComparator extends ByteArrayComparable {
    ...
    public int compareTo(byte[] value, int offset, int length) {
        Short that = Bytes.toShort(value, offset, length);
        return this.shortValue.compareTo(that);
    }
     ...
     */
    boolean areSerializedFieldsEqual(ShortComparator other) {
        if (other == this) return true;
        if (!(other instanceof ShortComparator)) return false;
